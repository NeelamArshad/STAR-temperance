...
ggplot(pred_data, aes(x = kind, y = estimate)) +
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2, color = "#2980B9") +
  theme_minimal(base_size = 14) +
  labs(
    title = "Predicted Test Scores by Class Type",
    subtitle = "Predicted means with 95% confidence intervals from linear regression model",
    x = "Class Type",
    y = "Predicted Test Score",
    caption = "Estimates based on fitted regression model. Error bars represent 95% confidence intervals."
  ) +
  theme(
...
```{r}
plot_predictions(fit_cs, by = c("race", "kind"))
...
```{r}
pred_data2 <- plot_predictions(fit_cs, by = c("race", "kind"), draw = FALSE)
pred_data2
```
...
ggplot(pred_data2, aes(x = kind, y = estimate, color = race, group = race)) +
  geom_point(size = 3) +
  geom_line(linewidth = 1) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Predicted Grade 4 Math Scores by Class Type and Race",
    subtitle = "Model-based predictions with 95% confidence intervals",
    x = "Class Type",
    y = "Predicted Math Score",
    color = "Race",
    caption = "Estimates from a linear regression model with interaction between race and class type.\nError bars represent 95% confidence intervals."
  ) +
  theme(
    plot.title = element_text(face = "bold"),
    axis.text.x = element_text(angle = 15, hjust = 1),
...
We are trying to understand student's performance in small classes when compared to large classes based on g4math scores, accounting for race. 
